...
func TestPreflightDataStore(t *testing.T) {
	...
	var dataCache []byte
	ctx := context.TODO()
	mockStore.EXPECT().Store(ctx, "/preflight/1/10", gomock.Any(), &store.Headers{
	...
	mockStore.EXPECT().Load(ctx, "/preflight/1/10", gomock.Any()).Return(io.NopCloser(bytes.NewReader(dataCache)), nil)
